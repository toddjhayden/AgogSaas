...
export class RecommendationPublisherService {
  ...
  private async processRecommendation(rec: Recommendation, domain: string): Promise<void> {
    ...
    // Check 2: Title-based duplicate check (prevents same feature being created repeatedly)
    // This is critical because each run generates new reqNumbers for the same features
    let titleExists = false;
    if (this.useCloudApi && this.apiClient) {
      titleExists = await this.apiClient.requestExistsByTitle(rec.title);
    } else if (this.useLocalDb && this.db) {
      const existingByTitle = await this.db.queryOne<{ id: string }>(
        'SELECT id FROM owner_requests WHERE title = $1',
        [rec.title]
      );
      titleExists = !!existingByTitle;
    }

    if (titleExists) {
      console.log(`[RecommendationPublisher] Title duplicate detected: "${rec.title}" - skipping`);
      return;
    }

    // Check 3: Semantic similarity check using embeddings
    ...
    // Validate that we have actual requirements, not empty/malformed data
    const validRequirements = Array.isArray(rec.requirements)
      ? rec.requirements.filter(r => r && typeof r === 'string' && r.trim().length > 0)
      : [];

    if (validRequirements.length === 0) {
      console.log(`[RecommendationPublisher] Skipping "${rec.title}" - no valid requirements provided`);
      return;
    }

    const description = validRequirements.join('\n- ');
    const fullDescription = `${rec.businessValue || rec.title}\n\n**Requirements:**\n- ${description}`;
