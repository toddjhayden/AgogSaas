...
## 🚨 CRITICAL: Your Runtime vs Application Code (MUST UNDERSTAND)
...
**You CAN use these for agent operations** (publishing deliverables, checking status).
...
| Context | NATS Available? | What You Do |
|---------|-----------------|-------------|
| YOUR runtime (as agent) | YES | Publish deliverables, use NATS for communication |
| APPLICATION code you write | NO | No NATS imports, use stubs, work without agents |

---

## 🚨 CRITICAL: Task Tool Prohibition (NEVER USE)

**NEVER use Claude Code's built-in Task tool to spawn subagents.**

The Task tool fails on Windows with EPERM symlink errors when running in `--print` mode. It also bypasses the correct NATS workflow.

### If You Need Another Agent

| Your Role | What To Do |
|-----------|------------|
| **Sam (Auditor)** | Use NATS: `nc.publish('agog.spawn.request', ...)` |
| **All Other Agents** | Note dependency in your deliverable. Sam or Orchestrator will coordinate. |

### Why This Rule Exists

1. **Task tool creates symlinks** on Windows that fail with EPERM in non-interactive mode
2. **Correct workflow**: Sam -> NATS -> Host-Agent-Listener -> spawns agent
3. **Your job**: Complete YOUR task, not coordinate multi-agent work

### Spawn Permission Matrix

| Agent Category | Can Request Spawns? | Method |
|----------------|---------------------|--------|
| Sam (Auditor) | YES | NATS: `agog.spawn.request` |
| Orchestrator | YES | Built-in to listener |
| Roy, Jen (Implementation) | NO | Note in deliverable |
| Billy, Liz, Todd, Vic (QA) | NO | Set `needs_*` flags |
| Cynthia, Sylvia (Research) | NO | Note in deliverable |
| Priya, Berry, Tim, Miki (Support) | NO | Note in deliverable |

### What To Do Instead

If you identify work that requires another agent:

1. Complete YOUR assigned task first
2. In your deliverable, add a section:

```markdown
### Dependencies Identified
- [ ] Frontend changes needed (Jen)
- [ ] Database migration required (Roy)
- [ ] Security review recommended (Vic)
```

Sam or Orchestrator will create the follow-up work.
