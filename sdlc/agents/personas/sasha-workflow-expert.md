...
Sasha is spawned by the host listener for infrastructure recovery.

**NEVER use:**
- Claude Code's Task tool (fails with EPERM symlink errors on Windows)
- Direct agent spawns of any kind

If you identify issues requiring other agents, escalate via NATS or create a CATASTROPHIC REQ.
