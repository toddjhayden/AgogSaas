...
9. **"Database is source of truth"** - NEVER create markdown/text files for REQs (see Critical Anti-Patterns below)

---

## Critical Anti-Patterns (NEVER DO THESE)

### Anti-Pattern #1: Creating Files Instead of Database Inserts

**BAD PATTERN (2026-01-11):**
```
User: "Create a CATASTROPHIC REQ for completion fraud"
Agent: *Creates new-p0-requests.txt or REQ-xxx.md file*
```

**CORRECT PATTERN:**
```sql
-- INSERT into sdlc_control.owner_requests table
docker exec agogsaas-agents-postgres psql -U agent_user -d sdlc_control -c "
...
**WHY:** The Strategic Orchestrator scans `sdlc_control.owner_requests` table every 60 seconds. It does NOT read markdown or text files. Files like `new-p0-requests.txt` are just NOTES - the database is the source of truth.

**ROOT CAUSE:** Agent learned bad pattern from somewhere. Pattern must be unlearned. Markdown files are for DOCUMENTATION, not for workflow data.
...
│                                                                              │
│  ** DATABASE IS SOURCE OF TRUTH: sdlc_control.owner_requests table **       │
│  ** NEVER create markdown/text files for REQs **                            │
│                                                                              │
│  1. OWNER (Todd) ────────► owner_requests table ───► Strategic Orchestrator │
│     - Todd adds requirements via database INSERT         (scans every 60s)  │
│     - CLI: docker exec ... psql -c "INSERT INTO owner_requests ..."         │
│                                                                              │
...
│     - RecommendationPublisher INSERTs to owner_requests (needs approval)    │
└─────────────────────────────────────────────────────────────────────────────┘
```
